...
lazy val html2pdf = project.in(file("html2pdf"))

lazy val utils = project.in(file("utils")).dependsOn(system)

lazy val json = project.in(file("json")).dependsOn(utils)

lazy val es = project.in(file("es")).dependsOn(utils, json)

lazy val session = project.in(file("session")).dependsOn(es, json, utils)

lazy val mnotify = project.in(file("notify")).dependsOn(html2pdf, es, json, session, utils)

//lazy val mauth = project.in(file("auth")).dependsOn(html2pdf, es, json, session, utils)

lazy val mirakl = project.in(file("mirakl")).dependsOn(system, json)

lazy val common = project.in(file("common")).dependsOn(json)

lazy val core = project.in(file("core")).dependsOn(system, /*mauth,*/ mnotify, html2pdf, es, json, session, utils, template, common, mirakl)

//lazy val mogobizRun = project.in(file("run")).dependsOn(mogopayCore)

//lazy val mogobizLaunch = project.in(file("launch")).dependsOn(mogobizRun, mogopayCore)

lazy val launch = project.in(file("launch")).dependsOn(core)

//lazy val mogobizSelenium = project.in(file("selenium")).dependsOn(mogobizLaunch)
...
//  mogobizLaunch,
  launch //,
//  mogobizSelenium
  ).enablePlugins(GitVersioning, GitBranchPrompt /*, BuildInfoPlugin*/).settings(
